...
for country_name in country_names:
    ...
    print(" - Applying exclusions - Slope above 8.53 degrees...")
    ec.excludeRasterType(os.path.join(data_path, f'Laos_slope_excluded_wind.tif'), value=1, prewarp=True)
    current_time = time.time() - start_time
    print(f"   Done! Time elapsed so far: {current_time:.4f} seconds")
    ...
    ec.save(os.path.join(output_dir,  f'{country_name}_wind_exclusions.tif'), overwrite=True)
    current_time = time.time() - start_time
    print(f"   Done! Time elapsed so far: {current_time:.4f} seconds")
    
    print(" - Distributing turbines and saving placements as .shp...")
    ec.distributeItems(separation=(d * 10, d * 4), axialDirection=180,
                       output=os.path.join(output_dir, f'{country_name}_turbine_placements.shp'))
    current_time = time.time() - start_time
    print(f"   Done! Time elapsed so far: {current_time:.4f} seconds")
